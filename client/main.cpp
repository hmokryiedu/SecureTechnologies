...
// Допоміжна функція для генерації правильного алфавіту (Windows-1251)
std::string GenerateFullCharset() {
    std::string s = "";
    ...
    // 3. Спецсимволи (додаємо апостроф, як було у вас, та інші)
    s += "'"; 
    s += "!_@#$"; 

    // 4. Кирилиця (Windows-1251)
    // Основний діапазон (А-я)
    for (int i = 192; i <= 255; ++i) {
        s += (char)i;
    }
    // Українські літери ("і", "ї", "є", "ґ" та великі)
    s += (char)178; s += (char)179; // І, і
    s += (char)170; s += (char)186; // Є, є
    s += (char)175; s += (char)191; // Ї, ї
    s += (char)165; s += (char)180; // Ґ, ґ
...
void ModeBruteForce() {
    ...
    switch (alphabetChoice) {
        ...
        case 2:
            ...
            break;
        case 3:
            // Full alphabet with Cyrillic (Generated Programmatically for CP1251)
            alphabet = GenerateFullCharset();
            break;
        default:
            Console::PrintError("Invalid choice!");
            return;
